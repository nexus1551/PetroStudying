...
70.80.81.82       `! Это пример IP адресса`
...
Заходим в `cmd` пишем `ssh-keygen` и после нажимаем `Enter`

У вас в `C:/Пользователи/Ваш_пользователь/` создалась папка `.ssh`, туда заходим и открываем файл `id_ed25519.pub` *ИМЕННО PUB!* Второй файл можно даже не трогать, он вам не нужен будет

Заходим на сервер по первому методу и на пользователе `user1` вводим `ssh-keygen` и нажимем *Enter*;

У вас создалась папка невидимая, чтобы её открыть введите `cd /home/user1` а после `cd .ssh` и `ls` чтобы увидеть все файлы
